...
create table email_addresses (
  ...
  obfuscation obfuscation_kind not null default 'none',
  source_value text,
  ...
  constraint email_addresses_has_provenance check (source_page_id is not null),
  ...
  -- A shared inbox has a null person_id but a real school or district, so the
  -- key must treat those nulls as comparable.
  constraint email_addresses_unique_per_person unique nulls not distinct (person_id, school_id, district_id, address_normalized)
);
...
create table email_candidates (
  ...
  -- Confidence in the inference. Kept in a separate column from
  -- validation_status so that "we think this is the pattern" can never be read
  -- as "a provider confirmed this mailbox".
  confidence numeric(4, 3) not null default 0,
  ...
  constraint email_candidates_unique_per_person unique (person_id, address)
);
